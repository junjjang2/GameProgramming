...
class Tile(GameObject):
    def __init__(self, canvas, x, y, number):
        ...
        item = canvas.create_rectangle(x - self.size, y - self.size, x + self.size, y + self.size, fill='blue', tag='tile')
    ...
    def draw(self, symbol):
        if symbol == "Flag":
            ...
            self.canvas.create_text(self.canvas.coords(self.item)[0]+self.size, self.canvas.coords(self.item)[1]+self.size, text=self.number)
...
class Game(tk.Frame):
    ...
    def game_loop(self):
        ...
        print(left_tiles, " ", self.levelsetting[self.level][2] - len([x for x in self.canvas.find_withtag('tile') if self.items[x].isopen() if self.items[x].get_number() is -1]))
        if left_tiles == self.levelsetting[self.level][2] - \
            len([x for x in self.canvas.find_withtag('tile') if self.items[x].isopen() if self.items[x].get_number() is -1]):
            for i in range(1, self.levelsetting[self.level][0] * self.levelsetting[self.level][1] + 1):
                self.items[i].open()
    ...
    def find_zero(self, idx):
        #print(idx, self.items[idx].get_number())
